...
async def test_startup_heartbeat_ignores_last_heartbeat_interval(monkeypatch: pytest.MonkeyPatch):
    now_ts = 1700002000
    storage_values = {
        scheduler_mod._HEARTBEAT_TS_KEY: now_ts - 300,
    }
    ...
    await scheduler_mod.ConsoleSyncScheduler.send_startup_heartbeat()

    assert called["hb"] == 1
    assert storage_values[scheduler_mod._HEARTBEAT_TS_KEY] == now_ts
