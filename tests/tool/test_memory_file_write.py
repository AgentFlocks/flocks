...
def test_daily_memory_rejects_agent_write(
    tmp_path: Path,
    exists: bool,
) -> None:
    memory_path = tmp_path / "memory" / "daily" / "2026-07-29.md"
    if exists:
        memory_path.parent.mkdir(parents=True, exist_ok=True)
        memory_path.write_text("existing\n", encoding="utf-8")
    ...
    assert error is not None
    assert "Session lifecycle" in error
...
@pytest.mark.asyncio
async def test_edit_rejects_daily_memory_before_permission(
    tmp_path: Path,
) -> None:
    memory_path = tmp_path / "memory" / "daily" / "2026-07-29.md"
    memory_path.parent.mkdir(parents=True)
    ...
    assert result.success is False
    assert "Session lifecycle" in (result.error or "")
    assert memory_path.read_text(encoding="utf-8") == "existing\n"
    assert permission_requested is False
