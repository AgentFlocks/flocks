...
@pytest.mark.asyncio
async def test_turn_prompt_context_reuses_runtime_snapshot(monkeypatch):
    runner = _make_runner("ses_runtime_prompt_snapshot")
    agent = _make_agent(name="rex")
    last_user = UserMessageInfo(
        id="msg_runtime_prompt_snapshot",
        sessionID=runner.session.id,
        role="user",
        time={"created": 1_000},
        agent="rex",
    ...
    config = SimpleNamespace(
        ...
        model_dump=lambda **_kwargs: {"sandbox": {"enabled": True}},
    )
    sandbox = AsyncMock(return_value="sandbox prompt")
    channel = AsyncMock(return_value="channel prompt")
    device = AsyncMock(return_value="device prompt")
    catalog = MagicMock(return_value="tool catalog")

    monkeypatch.setattr("flocks.config.Config.get", AsyncMock(return_value=config))
    monkeypatch.setattr("flocks.project.instance.Instance.get_worktree", lambda: "/tmp")
    monkeypatch.setattr("flocks.tool.device.store.device_revision", lambda: 3)
    monkeypatch.setattr(runner_mod.ToolRegistry, "revision", lambda: 7)
    monkeypatch.setattr(runner, "_build_sandbox_prompt", sandbox)
    monkeypatch.setattr(runner, "_build_channel_context_prompt", channel)
    monkeypatch.setattr(runner, "_build_device_asset_hint", device)
    monkeypatch.setattr(runner, "_build_tool_catalog_prompt", catalog)

    first = await runner._build_turn_prompt_context(
        agent=agent,
        messages=[last_user],
        last_user=last_user,
        tools=[],
    )
    second = await runner._build_turn_prompt_context(
        agent=agent,
        messages=[last_user],
        last_user=last_user,
        tools=[],
    ...
    assert first == second
    assert first.tool_revision == 7
    assert first.device_revision == 3
    assert first.config_instructions == ("rules.md",)
...
@pytest.mark.asyncio
async def test_minimal_turn_prompt_context_skips_runtime_sources(monkeypatch):
    runner = _make_runner("ses_minimal_runtime_prompt")
    runner._turn_additional_context = "delegated context"
    agent = _make_agent(name="rex-junior")
    last_user = UserMessageInfo(
        id="msg_minimal_runtime_prompt",
        sessionID=runner.session.id,
        role="user",
        time={"created": 1_000},
        agent="rex-junior",
    ...
    context = await runner._build_turn_prompt_context(
        agent=agent,
        messages=[last_user],
        last_user=last_user,
        tools=[],
    ...
    assert context.minimal_prompt is True
    assert context.additional_context == "delegated context"
    sandbox.assert_not_awaited()
    channel.assert_not_awaited()
    device.assert_not_awaited()

@pytest.mark.asyncio
async def test_process_step_passes_device_hint_in_turn_prompt_context(monkeypatch):
    ...
    monkeypatch.setattr(runner_mod.Provider, "get", lambda provider_id: provider)
    monkeypatch.setattr(runner_mod.Provider, "apply_config", AsyncMock(return_value=None))
    monkeypatch.setattr(runner_mod.SessionPrompt, "build_system_prompt_blocks", build_system_prompts)
    monkeypatch.setattr(runner, "_build_callable_tool_schema", AsyncMock(return_value=[]))
