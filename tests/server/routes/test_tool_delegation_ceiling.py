...
@pytest.mark.asyncio
async def test_http_root_uses_session_agent_when_request_omits_agent(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    isolated_storage,
) -> None:
    ...
    resolve_agent = AsyncMock(return_value="target-agent")
    builder = AsyncMock(return_value={"parent_ceiling": {"tools": ["read"]}})
    monkeypatch.setattr(
        "flocks.security.execution_context.resolve_execution_agent",
        resolve_agent,
    ...
    resolve_agent.assert_awaited_once_with(None, "target-agent")
    assert builder.await_args.kwargs["agent_name"] == "target-agent"
...
@pytest.mark.asyncio
async def test_http_root_rejects_unknown_agent_when_session_disappears_before_context(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    isolated_storage,
) -> None:
    ...
    session_lookup = AsyncMock(side_effect=[root_session, None])
    execute = AsyncMock()
    ...
    with pytest.raises(HTTPException) as exc_info:
        await _execute_with_http_context(
            tool_name="b4_http_context_tool",
            tool_info=_tool_info(),
            params={},
            session_id=root_session.id,
