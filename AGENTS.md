...
每个项目持续维护三份文档：**需求与产品设计文档、技术方案文档、组件实现说明与变更总结**。需求与产品设计合并在同一份文档中，不因每次迭代另建一套口径。

- 每次新增或调整需求、修改功能入口或位置、改变交互/业务逻辑、数据与接口、部署方式或验证结论，都必须主动同步这三份文档，无需用户再次提醒。
- 需求与产品设计记录当前需求、范围、用户操作、状态与验收标准；技术方案记录实际结构、接口、数据、状态流转和约束；第三份文档解释组件核心逻辑、任务核心步骤，以及每一步对应的代码文件和关键函数，并归纳为什么改、改了什么、影响范围、验证结果与遗留事项。
- 第三份文档必须能作为理解代码的入口，随实现维护流程与文件导航，不得只写变更日志。源码位置使用仓库相对路径，关键函数以实际代码为准。
- 变更开始时核对需求和方案，完成时与实际代码和验证结果对照更新。未受影响的文档也要核对并注明本轮无实质变化，不能跳过核对。文档同步完成前，不把本次变更报告为全部完成。
- 维护当前有效版本，修正已失效的正文，不只在末尾追加互相矛盾的记录。明确区分已实现、已验证、待实现与待验证；不同轮次重复执行的测试不能累加成新增覆盖。
- 文档继续遵守上方输出目录约定。跨日期时从最近的有效版本延续到执行当日目录，保留前一版引用；不得在仓库功能目录创建报告，也不得硬编码用户绝对路径。
- 本项目“安全运营监测”的三份文档为 `Flocks主机安全监测插件产品设计文档.md`（含需求）、`Flocks主机安全监测插件技术方案-第一阶段.md`、`Flocks安全运营监测项目变更总结.md`，统一放在输出目录对应日期下并相互引用。其他验收日志、截图、风险评审属于支撑证据，不替代这三份主文档。
