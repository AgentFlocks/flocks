...
import { findActiveTabHref, nextTabAfterClose, resolveOpenTabs } from '@/utils/layoutTabs';
import OpenTabsBar, { type OpenTabsBarGroup, type OpenTabsBarTab } from './OpenTabsBar';
...
export default function Layout({ contentRoutes = appContentRoutes }: LayoutProps = {}) {
  ...
  const [closingTabHref, setClosingTabHref] = useState<string | null>(null);
  ...
  const navigation = useMemo<LayoutNavSection[]>(
    () => {
      ...
      // Scene workspaces (SOC today) are top-level menus next to the AI
      // workbench; their pages form the second-level menu.
      const sceneWorkspaceSections: LayoutNavSection[] = enabledWorkspaces
        ...
        .map((workspace) => ({
          ...
          accordionGroup: 'primary',
          workspace,
          items: workspacePageItems(workspace),
        }));
    ...
    [i18n.language, productName, webuiContractPages, webuiContractWorkspaces, workspacePageOrders, t],
  );
  ...
  useEffect(() => {
    ...
    if (closingTabHref === activeTabHref) return;
    if (closingTabHref !== null) setClosingTabHref(null);
  ...
  }, [activeTabHref, closingTabHref, currentLocationPath, setOpenTabRecords]);

  const openTabs = useMemo<OpenTabsBarTab[]>(() => {
    const tabs: OpenTabsBarTab[] = resolveOpenTabs(navItemsFlat, openTabRecords)
      .map((tab) => ({ href: tab.href, path: tab.path, name: tab.name, icon: tab.icon }));
    if (activeTabHref && activeTabHref !== closingTabHref && !tabs.some((tab) => tab.href === activeTabHref)) {
    ...
    return tabs;
  }, [activeTabHref, closingTabHref, currentLocationPath, navItemsFlat, openTabRecords]);

  const closedTabGroups = useMemo<OpenTabsBarGroup[]>(() => {
    const open = new Set(openTabs.map((tab) => tab.href));
    return navigation.map((section, index) => ({
      id: section.id ?? `section-${index}`,
      name: section.name,
      items: section.items
        .filter((item) => !open.has(item.href))
        .map((item) => ({ href: item.href, name: item.name, icon: item.icon })),
    }));
  }, [navigation, openTabs]);

  const closeTab = useCallback((href: string) => {
    const hrefs = openTabs.map((tab) => tab.href);
    setOpenTabRecords(openTabs.filter((tab) => tab.href !== href).map((tab) => ({ href: tab.href, path: tab.path })));
    if (href !== activeTabHref) return;
    setClosingTabHref(href);
    const nextHref = nextTabAfterClose(hrefs, href);
    const nextTab = nextHref ? openTabs.find((tab) => tab.href === nextHref) : undefined;
    navigate(nextTab ? nextTab.path : '/', { replace: true });
  }, [activeTabHref, navigate, openTabs, setOpenTabRecords]);
  ...
  // One pane per open tab stays mounted; a route that is not a sidebar entry
  ...
  const reorderTabs = useCallback((fromHref: string, toHref: string) => {
    const hrefs = openTabs.map((tab) => tab.href);
    const fromIndex = hrefs.indexOf(fromHref);
    const toIndex = hrefs.indexOf(toHref);
    if (fromIndex < 0 || toIndex < 0 || fromIndex === toIndex) return;
    setOpenTabRecords(moveItem(openTabs, fromIndex, toIndex).map((tab) => ({ href: tab.href, path: tab.path })));
  }, [openTabs, setOpenTabRecords]);
  ...
  const effectiveExpandedPrimaryNavSectionId = useMemo(() => {
    ...
    return routePrimaryNavSectionId ?? AI_WORKBENCH_NAV_SECTION_ID;
        ...
        <div className="flex flex-col h-full overflow-visible">
          ...
          <nav className={`flex-1 overflow-y-auto overflow-x-hidden py-4 ${collapsed ? 'px-2' : 'px-3'}`}>
            {navigation.map((section, sectionIndex) => {
              ...
              const showSocWorkspaceActions = !collapsed && isSocWorkspace(sectionWorkspace);
              return (
