...
def daemon_alive(name=None):
    try:
        c = ipc.connect(name or NAME, timeout=1.0)
        c.close()
        return True
...
def stop_remote_daemon(name="remote"):
    ...
    # restarts anything on its own; a follow-up `uv run python -m scripts.run`
...
def restart_daemon(name=None):
    ...
    `uv run python -m scripts.run` invocation, which auto-spawns a fresh daemon via
...
def sync_local_profile(profile_name, browser=None, cloud_profile_id=None, include_domains=None, exclude_domains=None):
...
def print_update_banner(out=None):
    ...
    print(
        "[browser-harness] agents: run `uv run python -m scripts.run --update -y` to upgrade and restart the daemon",
        file=out,
    )
...
def run_setup():
    ...
    if not _chrome_running():
        print("no Chrome/Edge process detected. please start your browser and rerun `uv run python -m scripts.run --setup`.")
        return 1
    ...
    print("run `uv run python -m scripts.run --doctor` for diagnostics.", file=sys.stderr)
    return 1
...
def run_doctor():
    ...
    row("chrome running", chrome, "" if chrome else "start chrome/edge and rerun `uv run python -m scripts.run --setup`")
    row("daemon alive", daemon, "" if daemon else "run `uv run python -m scripts.run --setup` to attach")
    ...
    row(
        "BROWSER_USE_API_KEY set", api_key, "" if api_key else "optional: needed only for cloud browsers / profile sync"
    )
...
def run_update(yes=False):
    ...
    if daemon_alive():
        if _prompt_yes("restart the running daemon so it picks up the new code?", default_yes=True, yes=yes):
            restart_daemon()
            print("daemon stopped; it will auto-restart on next `uv run python -m scripts.run` call.")
        else:
            print(
                "daemon left running on old code. run `uv run python -m scripts.run` and it'll use the new code after the daemon recycles."
            )
