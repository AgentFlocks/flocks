...
在调用任何 `tdp_*` 工具之前，**必须**先确定目标设备的 `device_id`：

1. **调用 `device_context`** 获取当前所有已接入的 TDP 设备列表及其 `device_id`
2. **按用户提到的设备名称匹配**：
   - 用户说"TDP v4""tdp-v4""v4 那台"→ 在列表里找名称含"v4"的设备，取其 `device_id`
   - 用户说"那台 TDP""默认 TDP"但未指定版本 → 若只有一台则直接使用；若有多台，**必须向用户确认**使用哪台
   - 用户指定了但列表中找不到匹配 → **告知用户没有找到该设备**，列出可用设备供选择
3. **所有后续工具调用必须带上 `device_id`**，即使该参数在 schema 中标注为可选

!!! important: 禁止在不明确 `device_id` 的情况下直接发起 `tdp_*` 工具调用，即使工具可以不传 `device_id`，这样会导致请求落到错误的设备上。
