...
// --- 持久脚本注入（Page.addScriptToEvaluateOnNewDocument） ---
// 每个 targetId 映射到一个 identifier，用于后续覆盖/清除
const persistentScripts = new Map(); // targetId -> [{identifier, source}]

async function injectPersistentScript(targetId, source, identifier = 'default') {
  const sid = await ensureSession(targetId);

  // 如果该 target 之前注册过同 identifier 的脚本，先移除
  const existing = persistentScripts.get(targetId) || [];
  const old = existing.find(s => s.identifier === identifier);
  if (old) {
    try { await sendCDP('Page.removeScriptToEvaluateOnNewDocument', { identifier: old.identifier }, sid); } catch {}
    existing.splice(existing.indexOf(old), 1);
  }

  // 注册持久注入
  const resp = await sendCDP('Page.addScriptToEvaluateOnNewDocument', { source }, sid);
  if (resp.result?.identifier) {
    existing.push({ identifier, source, scriptIdentifier: resp.result.identifier });
    persistentScripts.set(targetId, existing);
    return { success: true, identifier: resp.result.identifier };
  }
  throw new Error('注册持久脚本失败: ' + JSON.stringify(resp.error || resp.result));
}

async function removePersistentScript(targetId, identifier = 'default') {
  const sid = await ensureSession(targetId);
  const scripts = persistentScripts.get(targetId) || [];
  const entry = scripts.find(s => s.identifier === identifier);
  if (!entry) return { success: false, reason: 'not found' };

  try { await sendCDP('Page.removeScriptToEvaluateOnNewDocument', { identifier: entry.scriptIdentifier }, sid); } catch {}
  scripts.splice(scripts.indexOf(entry), 1);
  if (scripts.length === 0) persistentScripts.delete(targetId);
  return { success: true };
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /close?target=xxx - 关闭 tab
      ...
      persistentScripts.delete(q.target);
    ...
    // POST /inject?target=xxx&identifier=default — 持久注入脚本（页面导航不丢失）
    // body 为 JS 源代码，注册后会在每次新文档加载时自动注入
    else if (pathname === '/inject') {
      const source = await readBody(req);
      if (!source) {
        res.statusCode = 400;
        res.end(JSON.stringify({ error: 'POST body 需要 JS 源代码' }));
        return;
      }
      const identifier = q.identifier || 'default';
      const result = await injectPersistentScript(q.target, source, identifier);
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/inject?target=&identifier=': 'POST body=JS脚本 - 持久注入（导航不丢失）',
