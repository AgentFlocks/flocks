...
description:
  ...
  只要用户提到需要联网、搜索、访问网页、浏览器操作、网页自动化等需求，必须先加载本 skill。
---
...
其中 `<skill-dir>` 为本 skill 的安装路径（通常为 `~/.flocks/plugins/skills/web-access`）。
...
```bash
node "<skill-dir>/scripts/find-url.mjs" [关键词...] [--only bookmarks|history] [--limit N] [--since 1d|7h|YYYY-MM-DD] [--sort recent|visits]
```
...
- 拿到媒体资源 URL 后，公开资源可以直接下载到本地后用读取；需要登录态才可获取的资源才需要在浏览器内 navigate + screenshot。
