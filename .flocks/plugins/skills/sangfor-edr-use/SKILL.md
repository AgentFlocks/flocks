...
深信服 EDR 通过浏览器操作，没有 API 工具，因此设备定位方式与其他平台略有不同：

1. **调用 `device_context`** 获取当前所有已接入的 EDR 设备列表（含设备名称和访问地址）
2. **按用户提到的设备名称匹配**：
   - 用户说出了具体名称（如"EDR 生产""sangfor-edr-01"）→ 在列表里找名称匹配的设备，取其访问 URL
   - 用户未指定具体设备 → 若只有一台则直接使用；若有多台，**必须向用户确认**使用哪台
   - 设备列表中没有该 EDR 配置 → **询问用户 EDR 访问 URL**，不要擅自填写域名
3. 确认 EDR URL 后，进入浏览器模式使用指南

进入浏览器模式前，**必须确认 EDR URL**（通过上述步骤从设备列表获取，或向用户询问），然后阅读浏览器模式使用指南。
