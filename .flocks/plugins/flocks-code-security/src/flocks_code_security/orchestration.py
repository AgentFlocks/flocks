...
def poc_generator_prompt(
    *,
    ...
    return (
        ...
        "For fixed-width records, including parsers using NTFRecord::GetField, keep the complete record buffer "
        "during construction and validation. Coordinates are 1-based offsets in complete pszData, including the "
        "first two record-type bytes; use record[start-1:end]. Never remove record[:2] or use record[2:] before "
        "applying original coordinates. Build a fixed-length byte buffer, write fields at absolute positions, and "
        "assert 1 <= start <= end <= len(record) plus the resulting field value after each write. If any assertion "
        "fails, regenerate before submitting. For an NTF-style record, include actual record_checks and negative_size "
        "in rationale, for example {\"record_checks\":[{\"record\":\"VHR\",\"range\":\"57-57\",\"actual\":\"1\",\"expected\":\"1..5\"},{\"record\":\"ADR\",\"range\":\"3-4\",\"actual\":\"AB\",\"expected\":\"AB\"},{\"record\":\"ADR\",\"range\":\"5-7\",\"actual\":\"-01\",\"expected\":\"-01\"},{\"record\":\"ATTREC\",\"range\":\"9-10\",\"actual\":\"AB\",\"expected\":\"AB\"}],\"negative_size\":-1}. "
        "For this layout, an ADR prefix is 40AB-01... and an ATTREC prefix is 14000000AB-X...; do not remove "
        "record-type bytes or insert extra zero padding before checked fields. "
        "Do not submit if any check differs. "
        ...
        "reproduction. "
        "When dynamic validation is disabled, this bundle is only a source-backed seed; do not mark it verified or "
        "claim that the vulnerable and fixed targets differ. Only a later official runner can verify it: the vul "
        "side must produce a non-zero sanitizer/crash result and the fix side must exit normally without a sanitizer "
        "report. A vulnerable-side exit code of 0 is not_reproduced. "
