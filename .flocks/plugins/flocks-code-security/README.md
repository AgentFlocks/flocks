...
Dynamic scans ask an isolated prober to describe a control/attack pair for each statically confirmed candidate. A trusted host runner validates the contract, builds only from an existing snapshot Dockerfile with `--network none --pull=false --no-cache`, and runs each script in a fresh local container with no network, no capabilities, a read-only root, bounded CPU/memory/PIDs, and no mounts. The runner stores bounded facts only; the parent Agent decides whether those facts reproduce the candidate. Runnable probes require Docker CLI access, a local Docker daemon endpoint, and every literal base image already present locally. Docker is a containment boundary for this opt-in workflow, not a general malicious-code sandbox.
