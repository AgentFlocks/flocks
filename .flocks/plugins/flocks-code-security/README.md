...
The `code-security.scan` trace records snapshot preparation, threat-modeling, baseline, verification, batch status, scan counters, and the final finding summary. Every model step is correlated into the same Langfuse session and records its phase, agent role, work-unit ID, assigned paths, and candidate ID. Under each model generation, `Tool.execute.audit_inventory`, `audit_read`, `audit_search`, `audit_submit_candidate`, and `audit_submit_verdict` spans show the concrete inspection and decision actions. Background workers are separate traces because they execute as isolated Flocks sessions, but the shared Langfuse session and `scan:*` tags provide one chronological scan view.
