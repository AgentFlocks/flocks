...
| **Decoy Token** | A unique marker inserted into system prompts to detect if the LLM has been tricked into leaking its instructions |
...
- **Rebuff**: Open-source multi-layered prompt injection detection framework by ProtectAI combining heuristics, LLM-based detection, vector similarity, and decoy tokens
