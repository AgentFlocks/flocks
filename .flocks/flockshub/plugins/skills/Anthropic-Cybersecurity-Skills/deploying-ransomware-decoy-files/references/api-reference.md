# API Reference: Deploying Ransomware Decoy Files

## Decoy File Deployment

| Function | Parameters | Returns |
|----------|-----------|---------|
| `deploy_decoy_files(target_dirs, custom_files)` | List of directories, optional custom file dict | Manifest with deployed file paths and hashes |
...
| `verify_decoy_integrity(manifest_path)` | Path to manifest JSON | Dict with intact/modified/missing counts |
| `simulate_ransomware_test(manifest_path)` | Path to manifest JSON | List of test results |

## DecoyFileHandler Events

| Event | Handler Method | Trigger |
|-------|---------------|---------|
| File modified | `on_modified(event)` | Content change detected |
| File deleted | `on_deleted(event)` | Decoy file removed |
| File renamed | `on_moved(event)` | Decoy file renamed or moved |
...
| File | Automatic | Writes to `decoy_alerts.jsonl` |
...
- Elastic Security Labs: Ransomware Decoy Files: https://www.elastic.co/security-labs/ransomware-in-the-honeypot-how-we-capture-keys
