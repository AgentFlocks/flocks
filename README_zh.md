...
Flocks 支持多种安装方式，请按你的运行环境选择其中一种。

| 方式 | 适用场景 |
|---|---|
| 3.1 一行命令安装 | 推荐，最快完成安装，适合 macOS / Linux / Windows |
| 3.2 源码安装 | 适合需要查看或修改源码的用户 |
| 3.3 Windows EXE 安装包 | 适合 Windows x64 图形化安装 |
| 3.4 Docker 安装 | 适合容器化部署；暂不支持 agent-browser headed 模式 |
...
> **中国大陆用户**：默认推荐使用 Gitee 上的 `install_zh` 一键安装脚本。脚本会在当前目录下创建 `./flocks` 并完成安装。
...
安装完成后，可通过 **开始菜单** 或可选的 **桌面快捷方式** 启动。

也可以在**新开**的终端中执行以下命令，以便新的 `PATH` 等环境变量生效：
