...
async def get_fallback_providers() -> FallbackProvidersConfig:
    """Return the effective ordered fallback model list."""
...
async def set_fallback_providers(
    ...
    if override_source:
        raise HTTPException(
            ...
            detail=(
                "Fallback models are controlled by "
                f"{override_source} and cannot be changed from the WebUI"
            ),
        )
    ...
    disabled_providers = set(config.disabled_providers or [])
    ...
    for index, fallback in enumerate(body.fallback_providers):
        ...
        if not provider_id or not model_id:
            raise HTTPException(
                ...
                detail=(
                    f"Fallback at index {index} must include provider_id "
                    "and model_id"
                ),
            )
        ...
        if provider is None or not provider.is_configured():
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Fallback provider '{provider_id}' is not configured",
            )
