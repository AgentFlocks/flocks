...
async def move_tool(ctx: ToolContext, sourcePath: str, targetPath: str) -> ToolResult:
    try:
    ...
    return ToolResult(success=True, output=f"Moved {source} -> {target}", metadata={"source": source, "target": target})
