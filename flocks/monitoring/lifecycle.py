...
async def install(manifest):
    ...
    async with _lock:
        ...
        await upsert_task_specs([TaskSpec(
            dedup_key=key, title='安全运营监测', description='每十分钟查询分析、逐条邮件通知；下一轮解读责任人回信、标记并回查',
...
async def uninstall():
    async with _lock:
        for entry in await rows('SELECT * FROM monitor_installations WHERE scope=?', (COMPONENT_ID,)):
            ...
            await write('UPDATE monitor_mail_settings SET enabled=0 WHERE owner=? AND scope=?', (entry['owner'], COMPONENT_ID))
    ...
    async with _lock:
        ...
        async with connection() as db:
            await db.execute('UPDATE task_schedulers SET context=? WHERE id=?',
                             (encode({**scheduler.context, 'monitoring': policy.model_dump()}), scheduler.id))
